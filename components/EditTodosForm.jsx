...
export default function EditTodosForm({ id, title, description }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Failed to update To DO's");
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-[#E86B51]">
      <input
        ...
        value={newTitle}
        className="border border-slate-500 px-8 py-2 w-[800px] rounded-[15px]"
        type="text"
